...
import com.whx.tmall.comparator.*;
import com.whx.tmall.pojo.*;
import com.whx.tmall.service.*;
import com.whx.tmall.comparator.*;
import com.whx.tmall.service.*;
...
//-----------------------新方法使用shiro的Subject对象进行校验，校验成功后创建Session------------------------
        ...
        //将账号名和密码送入shiro   UsernamePasswordToken:用户名密码令牌 当中
...
//        9. 通过 Result 把这个 map 返回到浏览器去
        ...
        productImageService.setFirstProductImage(product);//设置预览图

        //将数据打包封装返回前端
        Map<String, Object> map = new HashMap<>();
        map.put("product", product);
        map.put("psv", propertyValues);//里面包含property对象
...
//        return Result.fail("未登录");
    ...
    public Object changeOrderItem(@PathVariable("pid") int pid, @RequestParam("num") int num, HttpSession session) {
    ...
    public Object createOrderItem(@RequestBody Order order, HttpSession session) {
//        1. 从session中获取user对象
...
//        9. 返回总金额 和oid方便跳转到付款页面
    ...
    public Object bought(HttpSession session) {
        ...
        //设置order底下的OrderItem的order为Null，避免重复json化。进入死循环
        orderService.removeFromOrderItem(orders);
        return orders;
    }
    ...
    public Object confirmPay(int oid) {
        ...
        orderService.removeFromOrderItem(order);
        return order;
    }
